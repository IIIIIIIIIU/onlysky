...
{
    ll up,down;
    node(ll up_=0,ll down_=0)
    {
        up=up_,down=down_;
    }
    node operator+(const node &pos)const
    {
        if(up==0&&pos.up==0) return node(0,0);
        if(up==0||down==0) return node(pos.up,pos.down);
        if(pos.up==0||pos.down==0) return node(up,down);
        node res;
        res.up=up*pos.down+pos.up*down;
        res.down=down*pos.down;
        ll tmp=gcd(res.up,res.down);
        res.up/=tmp,res.down/=tmp;
//      if(res.down<0)
//      {
//          ll daw=0;
//          cout<<pos.up<<' '<<pos.down<<endl;
//          cout<<up<<' '<<down<<endl;
//      }
        return res;
    }
    ...
    {
        node res(up,down*pos);
        if(res.up==0) return res;
        ll tmp=gcd(res.up,res.down);
        res.up/=tmp;
        res.down/=tmp;
        return res;
...
int main()
{
    ...
    for(int i=1;i<=x;i++)
        ...
        {
            cin>>op;
            if(dp[i][v].up==0||dp[i][v].down==0) continue;
            if(op[0]=='*')
            {
                dp[i+1][v]=dp[i+1][v]+(dp[i][v]*2);
                dp[i+1][v+1]=dp[i+1][v+1]+(dp[i][v]*2);
            }
            else dp[i+2][v+1]=dp[i+2][v+1]+dp[i][v];
        }
    /*
    for(ll i=1;i<=n+1;i++)
    {
        for(ll v=1;v<=i;v++)
        {
            printf("%d,%d ",dp[i][v].up,dp[i][v].down);
        }
        printf("\n");
    }
    */
//  printf("%lld",dp[n+1][m+1].up);
    cout<<dp[x+1][y+1].up;
    putchar('/');
    cout<<dp[x+1][y+1].down;
    return 0;
}
